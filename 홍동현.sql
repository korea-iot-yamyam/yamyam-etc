drop DATABASE IF EXISTS yamyam_daily;
create database yamyam_daily;
use yamyam_daily;
...
-- 일매출
SELECT 
    DATE(order_date) AS 주문일, 
    SUM(total_price) AS 일매출
FROM orders
where YEAR(order_date) = 2024 AND MONTH(order_date) = 11	-- 여기에 변수값으로 설정
GROUP BY DATE(order_date)
ORDER BY 주문일;

-- 월매출 (그 달에 얼마를 팔았는지 띄우는 월매출)
SELECT MONTH(order_date) AS 주문월, SUM(total_price) AS 월매출
FROM orders
WHERE YEAR(order_date) = 2024 AND MONTH(order_date) = 12 -- 특정 연, 월을 지정할 경우
GROUP BY MONTH(order_date);

-- 연매출 (그 해에 얼마를 팔았는지 띄우는 연매출)
SELECT 
    YEAR(order_date) AS 연도, 
    SUM(total_price) AS 연매출
FROM orders
